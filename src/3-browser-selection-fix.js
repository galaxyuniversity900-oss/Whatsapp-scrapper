...
class BrowserManager{
  ...
  static detectInstalledBrowsers(){const out={};for(const name of ['chromium','chrome','edge']){let p=BrowserManager._pathCandidates(name).find(BrowserManager._exists)||null;if(!p){const cmds=name==='chrome'?['google-chrome','google-chrome-stable','chrome']:name==='edge'?['microsoft-edge','microsoft-edge-stable','msedge']:['chromium','chromium-browser'];for(const c of cmds){p=BrowserManager._which(c);if(p)break}}out[name]={available:!!p,path:p};}return out}
  async getAvailableBrowsers(){return BrowserManager.detectInstalledBrowsers()}
