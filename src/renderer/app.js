...
const AI_BASE='http://127.0.0.1:8787';
async function aiApi(path,opt={}){const r=await fetch(AI_BASE+path,{headers:{'Content-Type':'application/json',...(opt.headers||{})},...opt});const j=await r.json().catch(()=>({}));if(!r.ok||j.ok===false)throw new Error(j.error||('HTTP '+r.status));return j}
async function aiLoadProviders(){const j=await aiApi('/api/ai/providers');const s=$('#aiChatProvider');if(s){s.innerHTML='';(j.providers||[]).forEach(p=>{const o=document.createElement('option');o.value=p.id;o.textContent=p.name+' · '+p.provider;s.appendChild(o)})}return j}
async function aiLoadModels(){try{const j=await aiApi('/api/ai/models');$('#aiModelCount').textContent=j.count+' models';$('#aiModels').innerHTML=(j.models||[]).map(m=>'<button class="ai-model-chip" onclick="document.querySelector(\'#aiChatModel\').value=\''+String(m.model).replace(/'/g,"\\'")+'\';document.querySelector(\'#aiChatProvider\').value=\''+String(m.provider).replace(/'/g,"\\'")+'\'">'+m.label+' · '+m.model+'</button>').join('');}catch(e){$('#aiModels').textContent='ERROR: '+e.message}}
async function aiSaveProvider(){try{const j=await aiApi('/api/ai/providers',{method:'POST',body:JSON.stringify({id:$('#aiProviderId').value,name:$('#aiProviderName').value,provider:$('#aiProviderType').value,baseUrl:$('#aiBaseUrl').value,model:$('#aiModel').value,apiKey:$('#aiApiKey').value})});$('#aiProviderResult').textContent=JSON.stringify(j,null,2);await aiLoadProviders()}catch(e){$('#aiProviderResult').textContent='ERROR: '+e.message}}
async function aiDetectLocal(){const probes=[['Ollama','http://127.0.0.1:11434/v1/models'],['LM Studio','http://127.0.0.1:1234/v1/models'],['vLLM','http://127.0.0.1:8000/v1/models'],['llama.cpp','http://127.0.0.1:8080/v1/models']];const rows=await Promise.all(probes.map(async x=>{try{const r=await fetch(x[1],{signal:AbortSignal.timeout(1500)});return {runtime:x[0],online:r.ok,status:r.status}}catch{return {runtime:x[0],online:false}}}));$('#aiLocalResult').textContent=JSON.stringify(rows,null,2);$('#aiStatus').textContent=rows.some(x=>x.online)?'Local AI runtime detected':'AI Hub ready';}
async function aiChat(){try{const j=await aiApi('/api/ai/chat',{method:'POST',body:JSON.stringify({id:$('#aiChatProvider').value,model:$('#aiChatModel').value,prompt:$('#aiPrompt').value,temperature:Number($('#aiTemperature').value),maxTokens:Number($('#aiMaxTokens').value)})});$('#aiChatResult').textContent=j.result.text||JSON.stringify(j.result.raw,null,2)}catch(e){$('#aiChatResult').textContent='ERROR: '+e.message}}
async function aiCompare(){try{const j=await aiApi('/api/ai/compare',{method:'POST',body:JSON.stringify({prompt:$('#aiPrompt').value,model:$('#aiChatModel').value,maxTokens:Number($('#aiMaxTokens').value)})});$('#aiChatResult').textContent=JSON.stringify(j.results,null,2)}catch(e){$('#aiChatResult').textContent='ERROR: '+e.message}}
setTimeout(()=>{aiLoadProviders();aiLoadModels();aiDetectLocal()},900);
